"""Optimized UI service for handling HTML content and UI-related logic."""

from typing import Dict, Any, Optional
from pathlib import Path


class OptimizedUIService:
    """Optimized service for handling UI-related functionality."""
    
    def __init__(self):
        self.base_template = self._get_optimized_template()
        self.recording_script = self._get_recording_script()
    
    def _get_optimized_template(self) -> str:
        """Get the optimized HTML template with session persistence and enhanced AI reply display."""
        return """
        <!DOCTYPE html>
        <html lang="zh-CN">
        <head>
            ...
            <script>
                ...
                function displayAIResponse(result) {
                    ...
                    if (result.data) {
                        ...
                        // 文本回复（当content为JSON字符串时，仅展示其中的text字段）
                        if (data.reply) {
                            ...
                            if (typeof replyContent === 'string' && replyContent.trim().startsWith('{')) {
                                try {
                                    parsedJson = JSON.parse(replyContent);
                            ...
                            if (parsedJson) {
                                ...
                                if (!data.expression && (parsedJson.expression || parsedJson.expressions)) {
                                    ...
                                    expressionDiv.innerHTML = `<strong>😊 机器人表情:</strong> ${exprStr}`;
                        ...
                        if (data.interaction_details && data.interaction_details.touch_zone) {
                            const actionDiv = document.createElement('div');
                            actionDiv.className = 'ai-action';
                            actionDiv.textContent = `机器人感受到您抚摸${data.interaction_details.touch_zone.name}，心情愉悦`;
                            aiContent.appendChild(actionDiv);
                        }
